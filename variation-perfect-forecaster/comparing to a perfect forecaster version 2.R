...
idealised_expected <- function(predicted_prob) {
...
bin_data <- function(df, group_by) {
  df |>
    ...
    summarise(
      ...
      mean_idealised = mean(idealised_bs)
    ) %>%
    ungroup()
}
...
calc_mean_scores <- function(df) {
  df |> 
    summarise(brier_score = mean(brier_score), 
              idealised = mean(idealised_bs))
}

# plot the histogram of actual and idealised scores
...
create_long_df <- function(df) {
  df |>
    rename("Actual Brier score" = brier_score, "Idealised expected Brier score" = idealised_bs) |>
...
rep_confidence <- lapply(reps, function(x) {
  ...
  df <- data.frame(
    ...
    mutate(
      ...
      idealised_bs = prediction - prediction^2
    ) 
  return(df)
})
...
binned_data_confidence |>
  rename("Actual Brier score" = mean_brier, "Idealised expected Brier score" = mean_idealised) |>
  pivot_longer(cols = c(`Actual Brier score`, `Idealised expected Brier score`), 
               values_to = "value", names_to = "score") |>
...
test[, .(mean_old = mean(idealised_bs), 
...
plot_df_confidence_strat |>
  ...
  geom_text(aes(x = 1, y = 0.27, 
            ...
            size = 2.5, 
            data = mean_scores_confidence_strat, inherit.aes = FALSE, hjust = 1) +
  facet_grid(confidence ~ difficulty)

ggsave("variation-perfect-forecaster/behaviour-confidence-strat.jpg", 
       width = 7, height = 8)
...
plot_df_noise_strat |>
  ...
  geom_text(aes(x = 1, y = 0.27, 
            ...
            size = 2.5, 
            data = mean_scores_noise_strat, inherit.aes = FALSE, hjust = 1) +
  facet_grid(noise ~ difficulty)

ggsave("variation-perfect-forecaster/behaviour-noise-strat.jpg", 
       width = 7, height = 8)
...
ggsave("variation-perfect-forecaster/behaviour-underconfident-noise-strat.jpg", 
       width = 7, height = 8)
